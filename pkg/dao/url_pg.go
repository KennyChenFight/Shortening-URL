...
func (p *PGUrlDAO) Create(originalURL string) (*URL, *business.Error) {
	...
	err := p.client.RunInTransaction(context.Background(), func(tx *pg.Tx) error {
		_, err := tx.Model((*URL)(nil)).Query(pg.Scan(&url.ID, &url.Original, &url.CreatedAt, &url.ExpiredAt), "INSERT INTO urls SELECT id, ?, ?, ? FROM keys FOR UPDATE SKIP LOCKED LIMIT 1 RETURNING id, original", originalURL, now, now.Add(expiredDuration))
		if err != nil {
			return err
		}
...
func (p *PGUrlDAO) Expire(num int) (int, *business.Error) {
	res, err := p.client.Model(&URL{}).
